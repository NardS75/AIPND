...
def calculates_results_stats(results_dic):
    ...
    # Calculate result statistics according to requirements
    # We use sum function with proper if conditions
    #Z
    results_stats['n_images'] = len(results_dic)
    #B
    results_stats['n_dogs_img'] = sum(results_dic[key][3] for key in results_dic)
    #A
    results_stats['n_correct_dogs'] = sum(1 if results_dic[key][3] == 1 and results_dic[key][4] == 1 else 0 for key in results_dic)
    #E
    results_stats['n_correct_breed'] = sum(1 if results_dic[key][3] == 1 and results_dic[key][2] == 1 else 0 for key in results_dic)
    #C
    results_stats['n_correct_notdogs'] = sum(1 if results_dic[key][3] == 0 and results_dic[key][4] == 0 else 0 for key in results_dic)
    #Y
    results_stats['n_match'] = sum(results_dic[key][2] for key in results_dic)
    #D
