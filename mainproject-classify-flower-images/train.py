...
def supported_models():
    ...
    '''
    return list(set(mhp.supported_models() + mhk.supported_models()))

def supported_optimizer():
    ''' Return a list of supported optimizer
    '''
    return list(set(mhp.supported_optimizer() + mhk.supported_optimizer()))
...
def main():
    ...
    if in_arg.backend == 'keras': 
        mhk.save_checkpoint(model, in_arg.save_dir, filename = in_arg.checkpoint)
    elif in_arg.backend == 'pytorch':  
        mhp.save_checkpoint(model, in_arg.save_dir, filename = in_arg.checkpoint)
    else:
...
def get_input_args():
    ...
    parser.add_argument('--arch', type = str, choices=supported_models(), default = 'densenet121',
    ...
    parser.add_argument('--optimizer', type = str, choices=supported_optimizer(), default = 'SGD',
                    help = "Set optimizer to be used (default: 'SGD')")
    parser.add_argument('--backend', type = str, choices=['pytorch', 'keras'], default = 'keras',
                    help = "Set backend to be used (default: 'keras')")    
    ...
    if in_arg.backend == 'keras':
        if in_arg.arch not in mhk.supported_models():
            error_list.append("train.py: error: argument: --arch: Keras backend supported models: {}".format(mhk.supported_models()))
    elif in_arg.backend == 'pytorch':
        if in_arg.arch not in mhp.supported_models():
            error_list.append("train.py: error: argument: --arch: PyTorch backend supported models: {}".format(mhp.supported_models()))

    # Check specific optimizer foreach backend       
    if in_arg.backend == 'keras':
        if in_arg.optimizer not in mhk.supported_optimizer():
            error_list.append("train.py: error: argument: --optimizer: Keras backend supported optimizers: {}".format(mhk.supported_optimizer()))
    elif in_arg.backend == 'pytorch':
        if in_arg.optimizer not in mhp.supported_optimizer():
            error_list.append("train.py: error: argument: --arch: PyTorch backend supported optimizers: {}".format(mhp.supported_optimizer()))
...
def model_training(in_arg):
    ...
    """
    if in_arg.backend == 'keras': 
        model = mhk.create_and_train(in_arg.data_dir,
                                arch = in_arg.arch,
        ...
        model = mhp.create_and_train(in_arg.data_dir,
                                arch = in_arg.arch,
                                hidden_units = in_arg.hidden_units,
                                epochs = in_arg.epochs,
                                learning_rate = in_arg.learning_rate,
